...
interface AboutOurProductSectionProps {
  products: Product[];
}

const AboutOurProductSection = ({ products }: AboutOurProductSectionProps) => {
    ...
    <section className="w-full h-[600px] relative overflow-hidden">
      ...
      {activeProduct && (
        <div className="absolute inset-0 z-20 bg-black/80 flex flex-col items-center justify-center">
          ...
          >
            <source src={activeProduct.videoSrc} type="video/mp4" />
          ...
          <div className="relative z-10 text-white text-center p-8 max-w-3xl">
            <h2 className="text-4xl font-bold mb-4">{activeProduct.title}</h2>
            <p className="text-lg mb-6">
              {activeProduct.description}
            </p>
            <button
              onClick={() => handleExplore(activeProduct.link)}
